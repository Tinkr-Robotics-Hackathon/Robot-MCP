...
class RobotConfig:
    ...
    # OpenCVCamera(0, fps=5, width=1920, height=1080, color_mode=rgb) wrist
    # OpenCVCamera(1, fps=30, width=1920, height=1080, color_mode=rgb) side
    # OpenCVCamera(2, fps=5, width=1280, height=960, color_mode=rgb) top
...
- side: at the side, looks at the robot
...
"""
    ...
    SHOULDER_MOUNT_OFFSET_MM: float = (
        32.0  # Example: Offset for shoulder joint from idealized zero
    )
    ELBOW_MOUNT_OFFSET_MM: float = (
        4.0  # Example: Offset for elbow joint from idealized zero
    )
